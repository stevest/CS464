...
(function (root, factory) {
  ...
  .factory('accessSearchResults', [function(){
    var accessSearchResults = {
      ...
      updateSearchResults: function(newResults){
        ...
        if(newResults){
          accessSearchResults.searchResults = newResults;
        } else {
          accessSearchResults.searchResults = [];
        }
      },
      updateSearchQuery: function(newQuery){
        if(newQuery){
          accessSearchResults.searchQuery = newQuery;
        } else {
          accessSearchResults.searchQuery = '';
        }
      },
    ...
    .directive('angucompleteAlt', ['$q', '$parse', '$http', '$sce', '$timeout', '$templateCache', 'accessSearchResults',
    function ($q, $parse, $http, $sce, $timeout, $templateCache, accessSearchResults) {
    // keyboard events
    var KEY_DW  = 40;
    var KEY_RT  = 39;
    var KEY_UP  = 38;
          ...
          if (which === KEY_UP || which === KEY_EN) {
            ...
            if (!scope.searchStr || scope.searchStr === '') {
              ...
              searchTimer = $timeout(function() {
                searchTimerComplete(scope.searchStr);
                //SSS
                accessSearchResults.updateSearchResults(scope.results);
                accessSearchResults.updateSearchQuery(scope.searchStr);
              }, scope.pause);
            }
